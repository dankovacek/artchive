...
**General Setup**
* For my webhost, Nearlyfreespeech.net, an 'index' file of some
sort must be included in the /public folder.
* There are a few ways to accomplish the above point, and
my approach is to create the shell script run.sh, which calls
only one function, run.py.
* In calling the 'app' module in run.py  (from protected import app),
...
* Registering the default_module blueprint imports the main
functions of the controller code, where routing occurs, where
models are called, and where the views are executed.
